...
namespace Fireflies.Utility.Reflection.Fasterflect.Probing; 
...
internal static class TypeConverter {
    ...
    internal static object ConvertTypes(Type targetType, Type sourceType, object value) {
        ...
        if(targetType == typeof(Type)) return Type.GetType(Convert.ToString(value));

        if(sourceType == typeof(Type) && targetType == typeof(string)) return sourceType.FullName;
    ...
    private static object ConvertEnums(Type targetType, Type sourceType, object value) {
        if(targetType.IsEnum) {
            if(sourceType == typeof(string)) {
                ...
                if(Enum.IsDefined(targetType, source)) return Enum.Parse(targetType, source);
            }
    ...
    public static object Get(Type targetType, XmlNode node) {
        if(targetType == typeof(XmlNode)) return node;
    ...
    public static object Get(Type targetType, XAttribute attribute) {
        if(targetType == typeof(XAttribute)) return attribute;
    ...
    public static object Get(Type targetType, XElement element) {
        if(targetType == typeof(XElement)) return element;
    ...
    public static object Get(Type targetType, string value) {
        ...
        try {
            if(targetType.IsEnum) return ConvertEnums(targetType, sourceType, value);

            if(targetType == typeof(Guid)) return ConvertGuids(targetType, sourceType, value);

            if(targetType == typeof(Type)) return ConvertTypes(targetType, sourceType, value);
    ...
    public static object Get(Type targetType, object value) {
        ...
        if(sourceType == typeof(string)) return Get(targetType, value as string);

        if(sourceType == typeof(XmlNode)) return Get(targetType, value as XmlNode);

        if(sourceType == typeof(XAttribute)) return Get(targetType, value as XAttribute);

        if(sourceType == typeof(XElement)) return Get(targetType, value as XElement);

        if(targetType.IsEnum || sourceType.IsEnum) return ConvertEnums(targetType, sourceType, value);

        if(targetType == typeof(Guid) || sourceType == typeof(Guid)) return ConvertGuids(targetType, sourceType, value);

        if(targetType == typeof(Type) || sourceType == typeof(Type)) return ConvertTypes(targetType, sourceType, value);
    ...
    public static Guid StringToGuid(string guid) {
        var charBuffer = guid.ToCharArray();
        var byteBuffer = new byte[16];
        var nCurrByte = 0;
        foreach(var currChar in charBuffer) byteBuffer[nCurrByte++] = (byte)currChar;
    ...
    public static string GuidToBinaryString(Guid guid) {
        ...
        foreach(var currByte in guid.ToByteArray()) sb.Append((char)currByte);
        return sb.ToString();
    }
    ...
    private static object ConvertGuids(Type targetType, Type sourceType, object sourceObj) {
        if(targetType == typeof(Guid)) {
            if(sourceType == typeof(string)) {
                var value = sourceObj as string;
                if(value != null && value.Length == 16) return StringToGuid(value);

                return value != null ? new Guid(value) : Guid.Empty;
            }

            if(sourceType == typeof(byte[])) return new Guid((byte[])sourceObj);
        } else if(sourceType == typeof(Guid)) {
            var g = (Guid)sourceObj;
            if(targetType == typeof(string)) return GuidToBinaryString(g);

            if(targetType == typeof(byte[])) return g.ToByteArray();
        }
