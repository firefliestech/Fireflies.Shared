...
internal static class MapFactory {
    ...
    /// </summary>
    private static readonly Cache<SourceInfo, MethodMap> ctorMapCache = new();
    ...
    public static MethodMap PrepareInvoke(Type type, string[] paramNames, Type[] parameterTypes,
        ...
        var sourceInfo = new SourceInfo(type, paramNames, parameterTypes);
        var map = ctorMapCache.Get(sourceInfo);
        if(map == null) {
    ...
    internal static MethodMap DetermineBestConstructorMatch(Type type, string[] paramNames, Type[] parameterTypes,
        object[] sampleParamValues) {
        var map = DetermineBestMatch(type.GetConstructors(), false, paramNames, parameterTypes, sampleParamValues);
    ...
    internal static MethodMap DetermineBestMethodMatch(IEnumerable<MethodBase> methods, bool mustUseAllParameters, string[] paramNames,
        ...
        var map = DetermineBestMatch(methods, mustUseAllParameters, paramNames, parameterTypes, sampleParamValues);
        if(map != null) return map;
    ...
    private static MethodMap DetermineBestMatch(IEnumerable<MethodBase> methods, bool mustUseAllParameters,
        ...
        foreach(var method in methods) {
            var map = CreateMap(method, paramNames, parameterTypes, sampleParamValues, mustUseAllParameters);
            if(map != null && map.IsValid) {
                var isBetter = bestMap == null;
