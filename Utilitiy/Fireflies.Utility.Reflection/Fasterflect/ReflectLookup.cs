...
/// <summary>
///     Helper class for looking up reflection based types such as <see cref="MemberInfo" /> and
...
public static class ReflectLookup {
    ...
    public static FieldInfo Field(Type type, string name, FasterflectFlags bindingFlags) {
        ...
        if(bindingFlags.IsAnySet(FasterflectFlags.PartialNameMatch | FasterflectFlags.TrimExplicitlyImplemented)) return Fields(type, bindingFlags, name).FirstOrDefault();
        ...
        var hasSpecialFlags = bindingFlags.IsAnySet(FasterflectFlags.ExcludeBackingMembers | FasterflectFlags.ExcludeExplicitlyImplemented | FasterflectFlags.ExcludeHiddenMembers);
        if(hasSpecialFlags) {
    ...
    /// <param name="names">
    ///     The optional list of names against which to filter the result. If this parameter is
    ...
    ///     The <see cref="BindingFlags" /> or <see cref="FasterflectFlags" /> combination used to define
    ///     the search behavior and result filtering.
    /// </param>
    /// <param name="names">
    ///     The optional list of names against which to filter the result. If this parameter is
    ///     <see langword="null" /> or empty no name filtering will be applied. The default behavior is to check for an exact,
    ///     case-sensitive match. Pass <see cref="FasterflectFlags.ExcludeExplicitlyImplemented" /> to exclude explicitly
    ///     implemented
    ///     interface members, <see cref="FasterflectFlags.PartialNameMatch" /> to locate by substring, and
    ///     <see cref="FasterflectFlags.IgnoreCase" />  to ignore case.
    /// </param>
    ...
    public static IList<FieldInfo> Fields(Type type, FasterflectFlags bindingFlags, params string[] names) {
        if(type == null || type == typeof(object)) return Constants.EmptyFieldInfoArray;

        var recurse = bindingFlags.IsNotSet(FasterflectFlags.DeclaredOnly);
        var hasNames = names != null && names.Length > 0;
        var hasSpecialFlags = bindingFlags.IsAnySet(FasterflectFlags.ExcludeBackingMembers | FasterflectFlags.ExcludeExplicitlyImplemented | FasterflectFlags.ExcludeHiddenMembers);

        if(!recurse && !hasNames && !hasSpecialFlags) return type.GetFields(bindingFlags) ?? Constants.EmptyFieldInfoArray;
    ...
    private static IList<FieldInfo> GetFields(Type type, FasterflectFlags bindingFlags) {
        ...
        if(!recurse) return type.GetFields(bindingFlags) ?? Constants.EmptyFieldInfoArray;
    ...
    public static PropertyInfo Property(Type type, string name, FasterflectFlags bindingFlags) {
        ...
        if(bindingFlags.IsAnySet(FasterflectFlags.PartialNameMatch | FasterflectFlags.TrimExplicitlyImplemented)) return type.Properties(bindingFlags, name).FirstOrDefault();

        var result = type.GetProperty(name, bindingFlags | FasterflectFlags.DeclaredOnly);
        if(result == null && bindingFlags.IsNotSet(FasterflectFlags.DeclaredOnly))
            if(type.BaseType != typeof(object) && type.BaseType != null)
                return type.BaseType.Property(name, bindingFlags);

        var hasSpecialFlags = bindingFlags.IsSet(FasterflectFlags.ExcludeExplicitlyImplemented);
        if(hasSpecialFlags) {
    ...
    /// <param name="names">
    ///     A list of names of properties to be retrieved. If this is <see langword="null" />,
    ///     all properties are returned.
    /// </param>
    /// <returns>
    ///     A list of all public properties on the type filted by <paramref name="names" />.
    ...
    public static IList<PropertyInfo> Properties(Type type, FasterflectFlags bindingFlags, params string[] names) {
        if(type == null || type == typeof(object)) return Constants.EmptyPropertyInfoArray;

        var recurse = bindingFlags.IsNotSet(FasterflectFlags.DeclaredOnly);
        var hasNames = names != null && names.Length > 0;
        var hasSpecialFlags = bindingFlags.IsAnySet(FasterflectFlags.ExcludeBackingMembers | FasterflectFlags.ExcludeExplicitlyImplemented | FasterflectFlags.ExcludeHiddenMembers);

        if(!recurse && !hasNames && !hasSpecialFlags) return type.GetProperties(bindingFlags) ?? Constants.EmptyPropertyInfoArray;
    ...
    private static IList<PropertyInfo> GetProperties(Type type, FasterflectFlags bindingFlags) {
        ...
        if(!recurse) return type.GetProperties(bindingFlags) ?? Constants.EmptyPropertyInfoArray;
    ...
    public static MemberInfo Member(Type type, string name, FasterflectFlags bindingFlags, MemberTypes memberTypes = MemberTypes.All) {
        ...
        for(;;) {
            if(bindingFlags.IsAnySet(FasterflectFlags.PartialNameMatch | FasterflectFlags.TrimExplicitlyImplemented)) return type.Members(memberTypes, bindingFlags, name).FirstOrDefault();
    ...
    public static IList<MemberInfo> Members(Type type, MemberTypes memberTypes, FasterflectFlags bindingFlags, params string[] names) {
        if(type == null || type == typeof(object)) return Constants.EmptyMemberInfoArray;

        var recurse = bindingFlags.IsNotSet(FasterflectFlags.DeclaredOnly);
        var hasNames = names != null && names.Length > 0;
        var hasSpecialFlags = bindingFlags.IsAnySet(FasterflectFlags.ExcludeBackingMembers | FasterflectFlags.ExcludeExplicitlyImplemented | FasterflectFlags.ExcludeHiddenMembers);

        if(!recurse && !hasNames && !hasSpecialFlags) return type.FindMembers(memberTypes, bindingFlags, null, null);
    ...
    private static IList<MemberInfo> GetMembers(Type type, MemberTypes memberTypes, FasterflectFlags bindingFlags) {
        var recurse = bindingFlags.IsNotSet(FasterflectFlags.DeclaredOnly);
        if(!recurse) return type.FindMembers(memberTypes, bindingFlags, null, null);
    ...
    ///     These are returned in the same order and a <see cref="MissingMemberException" /> is thrown if any member is
    ///     missing.
    /// </summary>
    /// <param name="type">The type to reflect on.</param>
    ...
    ///     filtered by the supplied <paramref name="names" /> list. These are returned in the same order and a
    ...
    /// <param name="type">The type on which to reflect.</param>
    /// <param name="name">
    ///     The name of the method to search for. This argument must be supplied. The
    ///     default behavior is to check for an exact, case-sensitive match. Pass
    ///     <see cref="FasterflectFlags.PartialNameMatch" />
    ///     to locate by substring and <see cref="FasterflectFlags.IgnoreCase" /> to ignore case.
    /// </param>
    /// <returns>
    ///     The specified method or null if no method was found. If there are multiple matches
    ///     due to method overloading the first found match will be returned.
    /// </returns>
    ...
    /// <param name="name">
    ///     The name of the method to search for. This argument must be supplied. The
    ///     default behavior is to check for an exact, case-sensitive match.
    /// </param>
    /// <param name="parameterTypes">
    ///     If this parameter is not null then only methods with the same
    ///     parameter signature will be included in the result.
    /// </param>
    /// <returns>
    ///     The specified method or null if no method was found. If there are multiple matches
    ///     due to method overloading the first found match will be returned.
    ...
    ///     by substring and <see cref="FasterflectFlags.IgnoreCase" /> to ignore case.
    /// </param>
    /// <param name="bindingFlags">
    ///     The <see cref="BindingFlags" /> or <see cref="FasterflectFlags" /> combination used to define
    ///     the search behavior and result filtering.
    ...
    /// </param>
    /// <param name="parameterTypes">
    ///     If this parameter is supplied then only methods with the same parameter signature
    ///     will be included in the result. The default behavior is to check only for assignment compatibility,
    ///     but this can be changed to exact matching by passing <see cref="FasterflectFlags.ExactBinding" />.
    /// </param>
    /// <param name="bindingFlags">
    ///     The <see cref="BindingFlags" /> or <see cref="FasterflectFlags" /> combination used to define
    ///     the search behavior and result filtering.
    ...
    public static MethodInfo Method(Type type, Type[] genericTypes, string name, Type[] parameterTypes, FasterflectFlags bindingFlags) {
        var hasTypes = parameterTypes != null;
        var hasGenericTypes = genericTypes != null && genericTypes.Length > 0;
        // we need to check all methods to do partial name matches or complex parameter binding
        var processAll = bindingFlags.IsAnySet(FasterflectFlags.PartialNameMatch | FasterflectFlags.TrimExplicitlyImplemented);
        processAll |= hasTypes && bindingFlags.IsSet(FasterflectFlags.IgnoreParameterModifiers);
        processAll |= hasGenericTypes;
        if(processAll) return type.Methods(genericTypes, parameterTypes, bindingFlags, name).FirstOrDefault().MakeGeneric(genericTypes);
    ...
    /// <param name="type">The type on which to reflect.</param>
    /// <param name="parameterTypes">
    ///     If this parameter is supplied then only methods with the same parameter
    ///     signature will be included in the result.
    /// </param>
    /// <param name="names">
    ///     The optional list of names against which to filter the result. If this parameter is
    ///     <see langword="null" /> or empty no name filtering will be applied. The default behavior is to check for an exact,
    ...
    ///     If this parameter is supplied then only methods with the same generic parameter
    ...
    public static IList<MethodInfo> Methods(Type type, Type[] genericTypes, Type[] parameterTypes, FasterflectFlags bindingFlags,
        params string[] names) {
        if(type == null || type == typeof(object)) return Constants.EmptyMethodInfoArray;
        ...
        if(!recurse && !hasNames && !hasTypes && !hasSpecialFlags) return type.GetMethods(bindingFlags) ?? Constants.EmptyMethodInfoArray;
    ...
    private static IList<MethodInfo> GetMethods(Type type, FasterflectFlags bindingFlags) {
        ...
        if(!recurse) return type.GetMethods(bindingFlags) ?? Constants.EmptyMethodInfoArray;
    ...
    public static Attribute Attribute(Enum provider, Type attributeType) {
        ...
        var info = type.Member(provider.ToString(), FasterflectFlags.StaticAnyVisibility | FasterflectFlags.DeclaredOnly);
    ...
    /// <summary>
    ///     Gets all <see cref="System.Attribute" />s of type <typeparamref name="T" /> associated with the
    ///     <paramref name="provider" />.
    /// </summary>
    /// <returns>A list of the attributes found on the source element. This value will never be null.</returns>
    ...
    /// <summary>
    ///     Gets the <see cref="System.Attribute" />s associated with the enumeration given in <paramref name="provider" />.
    ///     The resulting list of attributes can optionally be filtered by suppliying a list of
    ///     <paramref name="attributeTypes" />
    ///     to include.
    /// </summary>
    /// <returns>A list of the attributes found on the supplied source. This value will never be null.</returns>
    ...
    ///     The list of attribute types to look for. If this list is <see langword="null" /> or
    ///     empty an <see cref="ArgumentException" /> will be thrown.
    /// </param>
    ...
    /// <param name="attributeTypes">
    ///     The optional list of attribute types with which members should
    ...
    /// <param name="memberTypes">The <see cref="MemberTypes" /> to include in the search.</param>
    /// <param name="bindingFlags">
    ///     The <see cref="BindingFlags" /> or <see cref="FasterflectFlags" /> combination
    ///     used to define the search behavior and result filtering.
    /// </param>
    /// <returns>A list of all matching members on the type. This value will never be null.</returns>
    ...
    ///     The optional list of attribute types with which members should
    ///     be decorated. If this parameter is <see langword="null" /> or empty then all fields and properties
    ///     matching the given <paramref name="bindingFlags" /> will be included in the result.
    /// </param>
    /// <returns>A list of all matching members on the type. This value will never be null.</returns>
    ...
    ///     <paramref name="bindingFlags" /> will be included in the result.
    /// </param>
    /// <returns>A list of all matching fields on the type. This value will never be null.</returns>
    ...
    /// <summary>
    ///     Gets all constructors on the given <paramref name="type" /> that match the specified
    ///     <paramref name="bindingFlags" />.
    ///     The resulting list of members can optionally be filtered by supplying a list of
    ///     <paramref name="attributeTypes" />, in which case only members decorated with at least one of
    ///     these will be included.
    ...
    /// </summary>
    /// <returns>
    ///     An dictionary mapping all matching members to their associated attributes. This value
