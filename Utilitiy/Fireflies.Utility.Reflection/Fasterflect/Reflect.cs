...
using Fireflies.Utility.Reflection.Fasterflect.Emitter;
...
public static class Reflect {
    ...
    private static readonly Cache<MemberCallInfo, MemberGetter> Getters = new();
    private static readonly Cache<MemberCallInfo, MemberSetter> Setters = new();
    private static readonly Cache<CtorInfo, ConstructorInvoker> Constructors = new();
    private static readonly Cache<CallInfo, MethodInvoker> Methods = new();
    private static readonly Cache<Type, ArrayElementSetter> ArraySetters = new();
    private static readonly Cache<Type, ArrayElementGetter> ArrayGetters = new();
    private static readonly Cache<MultiSetCallInfo, MultiSetter> MultiSetters = new();
    private static readonly Cache<MapCallInfo, ObjectMapper> Mappers = new();
    ...
    internal static ConstructorInvoker Constructor(Type type, BindingFlags bindingFlags, ConstructorInfo ctor, Type[] parameterTypes) {
        ...
        var info = new CtorInfo(type, bindingFlags, parameterTypes);
        var value = Constructors.Get(info);
        if(value != null)
            return value;

        if(ctor == null && (!type.IsValueType || parameterTypes.Length != 0)) ctor = ReflectLookup.Constructor(type, bindingFlags, parameterTypes) ?? throw new MissingMemberException(type.FullName, "ctor()");
    ...
    ///     The <see cref="BindingFlags" /> or <see cref="FasterflectFlags" /> to use when searching for
    ...
    internal static MemberGetter Getter(Type type, string name, MemberTypes memberType, FasterflectFlags bindingFlags, MemberInfo memberInfo) {
        var info = new MemberCallInfo(type, name, memberType, bindingFlags);
        var value = Getters.Get(info);
        if(value != null)
            return value;
    ...
    /// <param name="name">The name of the <see cref="FieldInfo" />.</param>
    /// <param name="bindingFlags">
    ///     The <see cref="BindingFlags" /> or <see cref="FasterflectFlags" /> to filter the
    ///     <see cref="FieldInfo" />.
    /// </param>
    /// <returns>A <see cref="MemberGetter" /> which gets the value of the given <see cref="FieldInfo" />.</returns>
    ...
    public static MultiSetter MultiSetter(Type type, FasterflectFlags bindingFlags, params string[] memberNames) {
        var callInfo = new MultiSetCallInfo(type, bindingFlags, memberNames);
        var value = MultiSetters.Get(callInfo);
        if(value != null)
            return value;
    ...
    public static MultiSetter MultiSetter(Type type, IList<MemberInfo> members) {
        var callInfo = new MultiSetCallInfo(type, members);
        var value = MultiSetters.Get(callInfo);
        if(value != null)
            return value;
    ...
    /// <param name="type">The type which the indexer belongs to.</param>
    /// <param name="bindingFlags">
    ///     The <see cref="BindingFlags" /> or <see cref="FasterflectFlags" /> used to lookup the
    ///     indexer.
    /// </param>
    /// <param name="parameterTypes">The types of the indexer parameters (must be in the right order).</param>
    /// <returns>The delegate which can get the value of an indexer.</returns>
    ...
    ///     The types of the indexer parameters (must be in the right order), plus
    ...
    internal static MethodInvoker Method(Type type, string name, FasterflectFlags bindingFlags, MethodInfo? method, Type[]? genericTypes, Type[] parameterTypes) {
        ...
        var info = new CallInfo(type, name, bindingFlags, genericTypes, parameterTypes);
        var value = Methods.Get(info);
        if(value != null)
            return value;

        method ??= ReflectLookup.Method(type, genericTypes, name, parameterTypes, bindingFlags) ?? throw new MissingMethodException(type.FullName, name);
    ...
    public static MethodInvoker Method(MethodInfo method) {
        return Method(method.DeclaringType, method.Name, FasterflectFlags.StaticInstanceAnyVisibility, method, method.GetGenericArguments(), method.GetParameters().ToTypeArray());
    }
    ...
    /// <param name="parameterTypes">The <see cref="Type" />s of <see cref="MethodInfo" />'s parameters.</param>
    /// <returns>A <see cref="MethodInvoker" /> which invokes the given <see cref="MethodInfo" />.</returns>
    public static MethodInvoker Method(MethodInfo method, params Type[] parameterTypes) {
        return Method(method.DeclaringType, method.Name, FasterflectFlags.StaticInstanceAnyVisibility, null, null, parameterTypes);
    }
    ...
    /// <param name="parameterTypes">The <see cref="Type" />s of the <see cref="MethodInfo" />'s parameters.</param>
    /// <returns>A <see cref="MethodInvoker" /> which invokes the given <see cref="MethodInfo" />.</returns>
    public static MethodInvoker Method(MethodInfo method, Type[] genericTypes, params Type[] parameterTypes) {
        return Method(method.DeclaringType, method.Name, FasterflectFlags.StaticInstanceAnyVisibility, null, genericTypes, parameterTypes);
    }
    ...
    public static ArrayElementGetter ArrayGetter(Type arrayType) {
        var value = ArrayGetters.Get(arrayType);
        if(value != null)
            return value;
    ...
    public static ArrayElementSetter ArraySetter(Type arrayType) {
        var value = ArraySetters.Get(arrayType);
        if(value != null)
            return value;
    ...
    /// <summary>
    ///     Creates a delegate that can map values from fields and properties on the source object to fields and properties
    ///     with the
    ...
    /// <param name="names">
    ///     The optional list of member names against which to filter the members that are
    ///     to be mapped. If this parameter is an empty list then no name filtering will be applied. The default
    ///     behavior is to check for an exact, case-sensitive match.
    /// </param>
    ...
    public static ObjectMapper Mapper(Type sourceType, Type targetType, params string[] names) {
        var value = Mapper(sourceType, targetType, FasterflectFlags.InstanceAnyVisibility, names, Constants.EmptyStringArray);
        return value;
    }
    ...
    public static ObjectMapper Mapper(Type sourceType, Type targetType, string[] sourceNames, string[] targetNames) {
        var value = Mapper(sourceType, targetType, FasterflectFlags.InstanceAnyVisibility, sourceNames, targetNames);
        return value;
    }
    ...
    /// <param name="names">
    ///     The optional list of member names against which to filter the members that are
    ///     to be mapped. If this parameter is an empty string then no name filtering will be applied.
    /// </param>
    /// <returns>An <see cref="ObjectMapper" /> which sets the target using the matching source members.</returns>
    /// <returns></returns>
    public static ObjectMapper Mapper(Type sourceType, Type targetType, FasterflectFlags bindingFlags, params string[] names) {
        var value = Mapper(sourceType, targetType, bindingFlags, names, names);
        return value;
    }

    /// <summary>
    ///     Creates a delegate that can map values from fields and properties on the source object to fields and properties on
    ///     the target object.
    /// </summary>
    /// <param name="sourceType">The type of the source object.</param>
    /// <param name="targetType">The type of the target object.</param>
    ...
    internal static ObjectMapper Mapper(Type sourceType, Type targetType, FasterflectFlags bindingFlags, string[] sourceNames, string[] targetNames) {
        var info = new MapCallInfo(sourceType, targetType, bindingFlags, sourceNames, targetNames);
        var value = Mappers.Get(info);
        if(value != null)
            return value;
    ...
    public static T DeepClone<T>(T source) where T : class, new() {
        ...
        var clone = DeepCloneExtensions.DeepClone(source);
        return clone;
    }
    ...
    public static T ShallowClone<T>(T obj) where T : class {
        ...
        var clone = (T)inst?.Invoke(obj, null);
        return clone;
    }
