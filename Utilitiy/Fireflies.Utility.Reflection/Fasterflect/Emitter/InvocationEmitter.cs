        ...
        for(int i = 0, count = ParamTypes.Length; i < count; ++i) {
            ...
            if(paramType.IsByRef) {
                ...
                if(!parameters[i].IsOut) // no initialization necessary is 'out' parameter
                {
                    ...
                    if(type == typeof(object)) Gen.Emit(type.IsValueType ? OpCodes.Unbox_Any : OpCodes.Castclass, type);
    ...
    protected void PushParamsOrLocalsToStack(int paramArrayIndex) {
        ...
        for(int i = 0, count = ParamTypes.Length; i < count; ++i) {
            ...
            if(paramType.IsByRef) {
                ...
                Gen.Emit(OpCodes.Ldc_I4, i);
                Gen.Emit(OpCodes.Ldelem_Ref);
                if(paramType != typeof(object)) Gen.Emit(paramType.IsValueType ? OpCodes.Unbox_Any : OpCodes.Castclass, paramType);
            }
        }
    }
}