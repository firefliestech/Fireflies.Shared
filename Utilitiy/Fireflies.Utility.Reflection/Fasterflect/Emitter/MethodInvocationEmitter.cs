...
internal class MethodInvocationEmitter : InvocationEmitter {
    ...
    protected internal override Delegate CreateDelegate() {
        ...
        if(HasRefParam) {
            ...
            if(hasReturnType) stloc_s(startUsableLocalIndex); // result = <stack>;
        ...
        if(ShouldHandleInnerStruct) StoreLocalToInnerStruct((short)(startUsableLocalIndex + 1)); // ((ValueTypeHolder)this)).Value = tmpStr; 
        if(hasReturnType) {
            Gen.Emit(OpCodes.Ldloc, (short)startUsableLocalIndex); // push result;
            if(method.ReturnType.IsValueType) Gen.Emit(OpCodes.Box, method.ReturnType); // box result;
        } else {
