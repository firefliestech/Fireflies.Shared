...
internal class CallInfo {
        ...
        if(obj is CallInfo other &&
           ...
           BindingFlags == other.BindingFlags) {
            ...
            for(int i = 0, count = GenericTypes.Length; i < count; ++i)
                if(!GenericTypes[i].Equals(other.GenericTypes[i]))
                    return false;
    ...
    public override int GetHashCode() {
        var hashCode = -937504810;
        hashCode = hashCode * -1521134295 + TargetType.GetHashCode();
        hashCode = hashCode * -1521134295 + BindingFlags.GetHashCode();
        hashCode = hashCode * -1521134295 + Name.GetHashCode();
        for(int i = 0, count = ParameterTypes.Length; i < count; ++i) hashCode = hashCode * -1521134295 + ParameterTypes[i].GetHashCode();
        for(int i = 0, count = GenericTypes.Length; i < count; ++i) hashCode = hashCode * -1521134295 + GenericTypes[i].GetHashCode();
        return hashCode;
    }
}