...
internal class MultiSetEmitter : BaseEmitter {
    public MultiSetEmitter(Type targetType, IList<MemberInfo> members) {
        ...
        for(int i = 0, count = Members.Count; i < count; ++i) {
            var isstatic = Members[i].IsStatic();
            if(!isstatic) {
                IsStatic = false;
                return;
            }
    ...
    protected internal override Delegate CreateDelegate() {
        ...
        for(int i = 0, count = members.Count; i < count; ++i) {
            ...
            if(method is FieldInfo field) {
                if(!field.IsStatic) {
                    ...
                    else if(i != count - 1) Gen.Emit(OpCodes.Dup);
                }

                Gen.Emit(OpCodes.Ldarg_1);
                Gen.Emit(OpCodes.Ldc_I4, i);
                Gen.Emit(OpCodes.Ldelem_Ref);
                if(field.FieldType != typeof(object)) Gen.Emit(field.FieldType.IsValueType ? OpCodes.Unbox_Any : OpCodes.Castclass, field.FieldType);
                Gen.Emit(field.IsStatic ? OpCodes.Stsfld : OpCodes.Stfld, field); // (this|tmpStr).field = value-to-be-set
            } else {
                var property = (PropertyInfo)method;
                var setMethod = property.GetSetMethod(true) ?? throw new MemberAccessException(TargetType.FullName + "." + property.Name);
                ...
                Gen.Emit(OpCodes.Ldc_I4, i);
                Gen.Emit(OpCodes.Ldelem_Ref);
                if(property.PropertyType != typeof(object)) Gen.Emit(property.PropertyType.IsValueType ? OpCodes.Unbox_Any : OpCodes.Castclass, property.PropertyType);
        ...
        if(handleInnerStruct) StoreLocalToInnerStruct(0); // ((ValueTypeHolder)this)).Value = tmpStr
