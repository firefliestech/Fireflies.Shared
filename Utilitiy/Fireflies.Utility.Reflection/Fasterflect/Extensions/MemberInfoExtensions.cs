...
public static class MemberInfoExtensions {
    ...
    internal static object Get(this MemberInfo memberInfo) {
        var getter = Reflect.Getter(memberInfo);
    ...
    internal static void Set(this MemberInfo memberInfo, object value) {
        var setter = Reflect.Setter(memberInfo);
        setter(null, value);
    }
    ...
    /// </summary>
    /// <returns>
    ///     True for static fields, properties and methods and false for instance fields,
    ///     properties and methods. Throws an exception for all other <see cref="MemberTypes" />.
    /// </returns>
    public static bool IsStatic(this MemberInfo member) {
        ...
        var message = string.Format("Unable to determine IsStatic for member {0}.{1}" +
    ...
    /// </summary>
    /// <returns>
    ///     True for instance fields, properties and methods and false for static fields,
    ///     properties and methods. Throws an exception for all other <see cref="MemberTypes" />.
    /// </returns>
    ...
    internal static bool HasName(this MemberInfo member, string name) {
        var i = member.Name.Length > 0 && member.Name[0] == '_' ? 1 : 0;
        var j = name.Length > 0 && name[0] == '_' ? 1 : 0;
        if(name.Length - j != member.Name.Length - i)
            return false;

        for(var count = name.Length; j < count; ++i, ++j)
