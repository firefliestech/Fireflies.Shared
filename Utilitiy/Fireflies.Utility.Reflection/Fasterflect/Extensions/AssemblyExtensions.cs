...
public static class AssemblyExtensions {
    ...
    /// <param name="assembly">The assembly in which to look for types.</param>
    /// <param name="names">
    ///     An optional list of names against which to filter the result.  If this is
    ///     <see langword="null" /> or left empty, all types are returned.
    /// </param>
    /// <returns>A list of all matching types. This method never returns null.</returns>
    ...
    ///     The <see cref="BindingFlags" /> used to customize how results
    ///     are filters. If the <see cref="FasterflectFlags.PartialNameMatch" /> option is specified any name
    ///     comparisons will use <see cref="String.Contains(String)" /> instead of <see cref="String.Equals(string)" />.
    /// </param>
    /// <param name="names">
    ///     An optional list of names against which to filter the result.  If this is
    ///     <see langword="null" /> or left empty, all types are returned.
    ...
    public static IList<Type> Types(this Assembly assembly, FasterflectFlags bindingFlags, params string[] names) {
        ...
        var partialNameMatch = bindingFlags.IsSet(FasterflectFlags.PartialNameMatch);
