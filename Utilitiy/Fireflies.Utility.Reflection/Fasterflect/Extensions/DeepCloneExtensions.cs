...
internal static class DeepCloneExtensions {
    ...
    private static T CloneObject<T>(this T source, Dictionary<object, object> map) where T : class, new() {
        ...
        var values = fields.Select(f => CloneField(f, source, map)).ToArray();
        for(var i = 0; i < fields.Count; ++i) fields[i].Set(clone, values[i]);
        return clone;
    }
    ...
    private static object CloneField(FieldInfo field, object source, Dictionary<object, object> map) {
        var result = field.IsLiteral ? field.GetRawConstantValue() : field.Get(source);
        if(result == null || !result.ShouldClone()) return result;

        object clone;
        if(map.TryGetValue(result, out clone)) return clone;
    ...
    private static bool IsCalculated(this FieldInfo field, Type ownerType) {
        string[] dictionaryFields = { "keys", "values" };
