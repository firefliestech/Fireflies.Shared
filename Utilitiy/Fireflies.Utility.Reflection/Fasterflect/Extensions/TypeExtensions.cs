...
public static class TypeExtensions {
    ...
    public static bool IsFloatingPoint(this Type type) {
        var ty = Nullable.GetUnderlyingType(type) ?? type;
    ...
    ///     <seealso cref="InheritsOrImplements" />
    ///     to check both interfaces and classes.
    /// </remarks>
    ...
    public static bool Implements(this Type type, Type interfaceType) {
        if(type == null || interfaceType == null || type == interfaceType)
            return false;
        if(interfaceType.IsGenericTypeDefinition && type.GetInterfaces().Where(t => t.IsGenericType).Select(t => t.GetGenericTypeDefinition()).Any(gt => gt == interfaceType)) return true;
    ...
    /// <summary>
    ///     Returns true if the supplied <paramref name="type" /> inherits from or implements the type
    ///     <typeparamref name="T" />.
    /// </summary>
    ...
    /// <summary>
    ///     Returns true of the supplied <paramref name="type" /> inherits from or implements the type
    ...
    private static readonly List<byte[]> tokens = new() {
    ...
    public static string Name(this Type type) {
        if(type.IsArray) return string.Format("{0}[]", ((MemberInfo)type.GetElementType()).Name);

        if(type.ContainsGenericParameters || type.IsGenericType) {
            if(type.BaseType == typeof(Nullable<>) || (type.BaseType == typeof(ValueType) && type.UnderlyingSystemType.Name.StartsWith("Nullable"))) return GetCSharpTypeName(type.GetGenericArguments().Single().Name) + "?";

            var index = type.Name.IndexOf("`");
            var genericTypeName = index > 0 ? type.Name.Substring(0, index) : type.Name;
